...
function Controller() {
    this.register = async (req, res) => {
        ...
        let userData = await TurfAdminModel.create(data);
    ...
    this.login = async (req, res) => {
        ...
        let userLogin = await TurfAdminModel.findOne(userQuery);
        if (!userLogin)
    ...
    this.forgetPassword = async (req, res) => {
        ...
        let updatePass = await TurfAdminModel.findOneAndUpdate({ email: data?.email, active: CONSTANT.STATUS.ACTIVE }, data, { new: true });
